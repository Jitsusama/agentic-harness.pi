...
| Alias types resolve or the miss is surfaced, not silent | 667 slack-message aliases, of which 621 resolve to no URL. Not a resolver fault: two value shapes are stored under one type, and `url()` expects `workspace/CHANNEL/pTIMESTAMP` while most values are `CHANNEL/TIMESTAMP` written from API data rather than parsed from a URL | 🔴 | Phase 3, RC-D and RC-H |
