...
| Every field mutation journalled and reversible, not only parent and status | `JournalChange.field` typed `parent \| status` | 🟢 | Phase 1 and the review follow-up, RC-K (`MutableField` widened, undo drop-a-skipped-op bug fixed; undo reverses parent, status, priority, rank and kind via a field-dispatched revert, so reparent re-ranking and reclassify are both reversible; document-stage reversal stays out because the journal keys on quest ids, and alias and tree reversal remain pending) |
...
| Rank canonical: no duplicate rank within a sibling set | 336 quests (93 percent) in a colliding rank group, which counted quests rather than sibling sets: the collisions fall into 8 groups, 6 of them holding nothing but sealed quests, and among the 49 live quests there was 1. Renumbered 2026-07-31; zero live collisions remain | 🟢 | Phase 2, RC-A and RC-H (create and priority moves append at the next free rank) |
...
| Cwd auto-load resolves only from scaffolded trees and the quest dir, not from adopted trees or git-worktree aliases | one checkout adopted by 8 quests plus 279 colliding aliases made a fresh session load an arbitrary quest | 🟢 | Session-liveness quest (`extensions/quest-workflow/lifecycle.ts` `restoreFromCwd` and `verbs/lifecycle.ts` `questIdFromCwd` filter to `origin: scaffolded`) |
| Tree-adopt guards a main working tree and a path another quest already tracks | adopting a shared main checkout is how the multi-quest bind formed | 🟢 | Session-liveness quest (`verbs/tree-ops.ts` `treeAdopt` refuses without `force`, via `isMainWorkingTree` and a cross-quest path scan) |
| A session active on more than one quest is repairable from its own log's authority | one session read active on up to 14 quests | 🟢 | Session-liveness quest (`session-audit` with `planSessionRepair`: preview by default, `force` applies the stray detaches, conflicted records left untouched) |
| A provably-dead session's stale membership is detachable, but only when a probe observed it gone, never by recency | crashed panes leave a session active for days | 🟢 | Session-liveness quest (`session-audit` with `planDeadSessions`: only identity-backed, probe-confirmed dead sessions are force-detached; recency-dead identity-less records are left) |
| Session liveness is observed at read time (tri-state), never a persisted recency guess | a quiet live pane read dead; a crashed shell read active for days | 🟢 | Session-liveness quest (`lib/internal/quest/session-liveness.ts` `buildLivenessSnapshot`/`deriveLiveness` over a process and terminal probe; identity captured on attach, never a cached verdict) |
...
| Alias types resolve or the miss is surfaced, not silent | `RefType.whyNoUrl` plus `whyRefHasNoUrl`, surfaced by `quest links`, tested in `tests/lib/refs/why-no-url.test.ts`. The cause was two value shapes under one type: `url()` expects `workspace/CHANNEL/pTIMESTAMP` while most values are `CHANNEL/TIMESTAMP` written from API data. No workspace is guessed, since inventing one produces a link that may go nowhere; the miss is named instead, and 639 refs now explain themselves. A value that is already a URL is returned as its own link, recovering 23 more | 🟢 | Phase 3, RC-D and RC-H |
| Discovery fails open and surfaces a malformed record instead of dropping it to invisible | 31 invisible out-of-vocabulary document stages, which appears to have counted the documents at `stage: build`: measured 2026-07-31 there were 3, two `verified` and one `final`. Discovery now reports the reason through its errors channel, and the 3 were migrated with the original word kept in `stageWas` | 🟢 | Phase 3, RC-D (`explainDocumentFrontMatter`) |
...
| One projection feeds both the human render and the agent result | `renderResult` in the quest-workflow index routes `show`, `who`, `links` and `ancestors` through `collapseText`, which previews the first line with an expand hint over the agent's own text rather than discarding the rest | 🟢 | The row described a collapse that had already been fixed. Reading the code to confirm it turned up an inline plural in `collapseText` itself, which is what extended `counts-are-said-properly` to a second spelling |
...
| A count is said properly, never `1 tree(s)` | n/a (shared UI) | 🟢 | `tests/package/counts-are-said-properly.test.ts` sweeps `lib/` and `extensions/` for both spellings, telling prose from a call so `push(s)` does not trip it. `lib/ui/count.ts` is the one pluralizer; review had a private copy and the other surfaces spelled `(s)` inline in forty-five places. The second spelling was added after this row was already green: banning `(s)` selects for `${n === 1 ? "" : "s"}`, and nineteen sites across ten files had reached for it while the gate reported the rule enforced. |
| A refusal states its corrective action, not a skill to read | n/a (enforcement) | 🟢 | `tests/package/refusals-are-actionable.test.ts`, plus a twin in the private package. Checks no block message hands the reader onward and that both required command forms appear literally. |
