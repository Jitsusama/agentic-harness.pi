...
| A count is said properly, never `1 tree(s)` | n/a (shared UI) | 🟢 | `tests/package/counts-are-said-properly.test.ts` sweeps `lib/` and `extensions/` for the shortcut, telling prose from a call so `push(s)` does not trip it. `lib/ui/count.ts` is the one pluralizer; review had a private copy and the other surfaces spelled `(s)` inline in forty-five places. |
