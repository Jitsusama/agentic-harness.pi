...
The tool does not validate the shape beyond it being an object. The
round reads these the same way it reads an answer and warns about what
it cannot use; a tool that argued with a reviewer about a missing field
would spend the reviewer's remaining budget on the argument.
