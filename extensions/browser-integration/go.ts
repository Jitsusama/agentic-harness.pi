...
export function registerGo(pi: ExtensionAPI, registry: SessionRegistry): void {
	pi.registerTool({
		...
		description:
			...
			"a url if you have one; kind 'close' disposes it. Navigating returns " +
			"the page's accessibility outline, so you see where you landed.",
		promptSnippet:
		...
		async execute(_id, params) {
			...
			if (params.kind === "navigate" && !params.url) {
				return refusal(name, params.kind, "navigate needs a url.");
			}

			const session = await registry.acquire(name);
			if (!params.url) {
				return answer(name, params.kind, `Opened session '${name}'.`);
			}
