 ...
 * touches: the stream has a line cap and the answer has a text cap, and
 * a file read straight off disk has neither. A finding is a paragraph,
 * so 64 KB is generous for one and a hundred is more than any reviewer
 * has ever raised. Whatever is refused stays on disk and the warning
 * says where, so this bounds what travels rather than what is kept.
 */
const MAX_JOURNAL_ENTRY_BYTES = 64 * 1024;
const MAX_JOURNAL_ENTRIES = 100;

/** Opens anything said about the journal, so the round can find it. */
const JOURNAL_SAYS = "Recorded findings:";
...
child = spawn(request.binary, request.args, {
	...
	env: {
		...
		...(request.paths.journalPath
			? { SUBAGENT_JOURNAL_PATH: request.paths.journalPath }
			: {}),
	},
});
...
heartbeatTimer = setInterval(() => {
	...
	writingLease = true;
	void writeJsonAtomic(request.paths.leasePath, lease("running")).finally(
		() => {
			writingLease = false;
		},
	);
...
async function readJournal() {
	...
	for (const line of raw.split("\n")) {
		...
		// The only channel a reviewer controls with no cap on it
		// anywhere else. A finding is a paragraph; a megabyte of one is
		// a reviewer pasting a file, and it would ride to the parent
		// past every limit the stream has. Counted in bytes, since a
		// string's length is UTF-16 units and a cap that means one
	...
	if (dropped > 0) {
		warn(
			`${JOURNAL_SAYS} ${dropped} could not be read back, most likely written as the reviewer was stopped`,
		);
	}
	if (tooBig > 0) {
		warn(
			`${JOURNAL_SAYS} ${tooBig} were larger than ${MAX_JOURNAL_ENTRY_BYTES} bytes and were left behind; they are still at ${path}`,
		);
	}
	if (tooMany > 0) {
		warn(
			`${JOURNAL_SAYS} ${tooMany} arrived past the limit of ${MAX_JOURNAL_ENTRIES} and were left behind; they are still at ${path}`,
		);
	}
	return entries;
}
