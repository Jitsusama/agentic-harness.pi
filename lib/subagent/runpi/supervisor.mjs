...
function captureAssistantMessage(event) {
	...
	// Everything below reads a finished turn. Usage on a partial is
	// the running total for the turn it belongs to, so accumulating
	// every update bills that turn once per update, and the stop
	// reason on a partial is not the turn's outcome yet.
