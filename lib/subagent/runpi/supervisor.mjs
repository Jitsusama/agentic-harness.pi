...
async function readJournal() {
	...
	try {
		...
		if (error?.code !== "ENOENT") {
			warn(
				`Could not read back what this reviewer recorded (${error?.code ?? error}); any findings it wrote down are still at ${path}`,
			);
		}
		return [];
	}
	...
	for (const line of raw.split("\n")) {
		...
		if (line.length > MAX_JOURNAL_ENTRY_BYTES) {
			oversized += 1;
			continue;
		}
	...
	if (dropped > 0) {
		warn(
			`${dropped} recorded finding${dropped === 1 ? "" : "s"} could not be read back, most likely written as the reviewer was stopped`,
		);
	}
	if (oversized > 0) {
		warn(
			`${oversized} recorded finding${oversized === 1 ? "" : "s"} were too large or too many to carry back; all of them are still at ${path}`,
		);
	}
	return entries;
}
