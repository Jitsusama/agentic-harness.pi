...
/**
 * The largest one recorded finding may be.
 *
 * The journal is the only channel a reviewer controls with no cap on
 * it anywhere else. A finding is a paragraph; a megabyte of one is a
 * reviewer pasting a file, and it would ride to the parent past every
 * limit the stream has. Counted in bytes, since a string's length is
 * UTF-16 units and a cap that means one thing for prose and another
 * for anything else is not a cap.
 */
