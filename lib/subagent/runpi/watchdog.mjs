 ...
 * its meaning. It is the only verdict here that expects the reviewer
 * to still be working: it says the run is fine and has been going long
 * enough that we would rather have its answer now than its
 * investigation later. Either real limit firing in the same tick is
 * the truer reason, so both are answered first.
 *
